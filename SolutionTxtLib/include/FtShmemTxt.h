//=============================================================================
//              |
// Headerfile   |   FtShmemTxt.h (original FtShmem.h)
//              |   Original:
...
namespace fischertechnik {
	namespace txt {
		namespace ids {
			...
			constexpr auto FIRMWARE_VER = 0x4406;      // was firmware version is 1.30 0x011e
			/// <summary>
			/// The 4 Motor ouputs
			/// </summary>
			enum Motor :uint8_t
		   ...
		   /// <summary>
			...
			/// </summary>
			constexpr uint8_t IZ_COUNTER = 4;          // number of counter
			constexpr uint8_t IZ_PWM_CHAN = 8;          // number of pwm channels
			...
			*/
			constexpr uint8_t I2C_SPEED_100_KHZ = 0;  /*!< I2C bus clock speed */
			constexpr uint8_t I2C_SPEED_400_KHZ = 1;  /*!< I2C bus clock speed */
		}
	}
}
using namespace fischertechnik::txt::ids;
//2020-06-20 update to TXT firmware 4.6.6
...
typedef struct ftX1info_s
{
	...
	char            dummy;
	uint32_t          SharedMemoryStart;
	uint32_t          AppAreaStart;
	uint32_t          AppAreaSize;
	FTX1_SHMVER     version;
} FTX1_SHMIFINFO;
...
{
	UINT8           pgm_state_req;        // enum PgmState    pgm_state_req;//Not in use on the TXT
	BOOL8           old_FtTransfer;
	char            dummy[2];
	BOOL8           motor[IZ_MOTOR];     // 0=single output O/O, 1=motor output M (default)
	UNI_CONFIG      uni[IZ_UNI_INPUT];   // Default digital 1
	CNT_CONFIG      cnt[IZ_COUNTER];     // Default 1
...
typedef struct ftX1input
{
	...
	INT16           display_button_left;    // TX-C
	INT16           display_button_right;   // TX-C
